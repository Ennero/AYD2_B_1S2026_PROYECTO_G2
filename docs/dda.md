...
| **Flujo Principal A (Emisión de Factura)** | 1. El Agente Financiero inicia el proceso en Generar y Emitir Factura FEL (CDU003.1). 2. El sistema ejecuta Validar Reglas Fiscales (CDU003.2) verificando montos. 3. El sistema envía el DTE al Certificador FEL, quien ejecuta Certificar DTE ante SAT (CDU003.3), validando, firmando digitalmente y devolviendo la certificación electrónica. 4. El sistema finaliza con Emitir Factura Física (CDU003.4), generando y enviando la representación gráfica (PDF). |
...
| **HU-12** | Como **Piloto**, quiero confirmar la entrega con evidencia fotográfica y firma de recepción, para cerrar la orden y dejarla lista para facturación. | Piloto | 1. Se captura al menos una fotografía de la entrega. 2. La orden cambia a estado "Entregada". 3. Se notifica al área financiera para iniciar la facturación. | RF-10 | Alta |
