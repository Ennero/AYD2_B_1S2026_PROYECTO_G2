...
| **RF-11** | CDU003 | Generación y Emisión de Factura FEL | Registro de descripción y fecha por Finanzas, emision de factura con validación fiscal, certificación DTE ante la SAT a través del Certificador FEL y generación de representación gráfica (PDF). | Agente Financiero, Certificador FEL | Alta |
...
| **Post Condiciones** | **Éxito:** Mercancía entregada, evidencia registrada y borrador de factura generado automáticamente para revisión financiera (en bandeja de descripción vacía). **Fallo:** Orden rechazada por crédito o detenida en patio por incumplimiento de normas de seguridad. |
...
| **Flujo Principal A (Emisión de Factura)** | 1. Cuando la entrega se confirma, el sistema genera automáticamente un borrador de factura sin descripción. 2. El Agente Financiero revisa el borrador, completa descripción/fecha e inicia el proceso en Generar y Emitir Factura FEL (CDU003.1). 3. El sistema ejecuta Validar Reglas Fiscales (CDU003.2) verificando montos. 4. El sistema envía el DTE al Certificador FEL, quien ejecuta Certificar DTE ante SAT (CDU003.3), validando, firmando digitalmente y devolviendo la certificación electrónica. 5. El sistema finaliza con Emitir Factura Física (CDU003.4), generando y enviando la representación gráfica (PDF). |
...
| **HU-12** | Como **Piloto**, quiero confirmar la entrega con evidencia fotográfica y firma de recepción, para cerrar la orden y dejar listo el borrador de facturación para Finanzas. | Piloto | 1. Se captura al menos una fotografía de la entrega. 2. La orden cambia a estado \"Entregada\". 3. Se genera automáticamente un borrador de factura (sin descripción) y se notifica al área financiera para revisarlo. | RF-10 | Alta |
